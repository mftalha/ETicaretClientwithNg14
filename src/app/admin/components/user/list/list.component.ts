...
export class ListComponent extends BaseComponent implements OnInit {
  ...
  async getUsers(){
    ...
    const allUsers: { totalUsersCount: number; users: List_User[] } = await
     this.userService.getAllUsers(this.paginator ? this.paginator.pageIndex : 0, this.paginator ?
    ...
    this.dataSource = new MatTableDataSource<List_User>(allUsers.users); // tabloya verdiğim veriler
